...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        if self.MODE in ['robust_ppo', 'radial_ppo', 'robust_ppo_stdv'] or self.MODE == 'adv_sa_ppo':
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        ...
        if (self.MODE in ['robust_ppo','radial_ppo'] or self.MODE == 'adv_sa_ppo') and not adversary_step and logging:
        ...
        if (self.MODE in ['robust_ppo','radial_ppo'] or 'robust_ppo' in self.MODE or self.MODE == 'adv_sa_ppo') and \
            (isinstance(self.policy_model, CtsPolicy) or isinstance(self.policy_model, CtsPolicy_stdv)) and not adversary_step:
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        if not policy_model.discrete:
            ...
            self.store.log_table_and_tb('optimization'+table_name_suffix, {
                # 'mean_std': mean_std,
                'mean_std': 1.11111111, ####### requires fix, no mean_std if stdv dependent on 

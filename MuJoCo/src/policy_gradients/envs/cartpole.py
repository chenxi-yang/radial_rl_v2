...
class CartPole(gym.Env):
    ...
    def step(self, action): # the dynamics for the real environment
    ...
    # The linear approximation of cartpole
    # def symbolic_dynamics(self, action, state):
    #     force = action.mul(self.force_mag)
    #     x = state.select_from_index(1, index0)
    #     x_dot = state.select_from_index(1, index1)
    #     theta = state.select_from_index(1, index2)
    #     theta_dot = state.select_from_index(1, index3)

    #     x = x.add(x_dot.mul(var(0.02)))
    #     x_dot = x_dot.add(force.mul(var(0.0195))).add(theta.mul(var(-0.0143)))
    #     theta = theta.add(theta_dot.mul(var(0.02)))
    #     theta_dot = theta_dot.add(force.mul(var(-0.0029)))
    #     symbolic_state = x.concatenate(x_dot.concatenate(theta.concatenate(theta_dot)))
    #     # self.symbolic_state.set_from_index(0, x.add(x_dot.mul(var(0.02))))
    #     # self.symbolic_state.set_from_index(1, x_dot.add(force.mul(var(0.0195))).add(theta.mul(var(-0.0143))))
    #     # self.symbolic_state.set_from_index(2, theta.add(theta_dot.mul(var(0.02))))
    #     # self.symbolic_state.set_from_index(3, theta_dot.add(force.mul(var(-0.0029))))
    #     return symbolic_state
    ...
    def reset(self):
        self.state = np.array([self.np_random.uniform(low=-0.01, high=0.01), \

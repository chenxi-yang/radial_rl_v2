...
def main(params):
    ...
    dynamics_params = ['load_dynamical_model', 'dynamics_enable', 'dynamics_epochs', 'dynamics_lr']
    ...
    if params['config_path']:
        ...
        params = override_json_params(params, json_params, excluded_params + sarsa_params + imit_params+dynamics_params)
    ...
    ## pass if stdv dependent on state

    if hasattr(p.policy_model, 'log_stdev'):
        ...
        if params['deterministic']:
            print('Policy runs in deterministic mode. Ignoring Gaussian noise.')
            p.policy_model.final_logstdv.bias.data[:] = -1000
    ...
    if params['sarsa_enable']:
        ...
        for i in range(num_episodes):
            ...
            try:
                ...
                ep_length, ep_reward, actions, action_means, states, kl_certificates = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=None)
    ...
    elif params['dynamics_enable']:
        num_epochs = params['dynamics_epochs']
        num_episodes = params['num_episodes']
        print('\n\n'+'Start collecting data\n'+'-'*80)
        for i in range(num_episodes):
            print('Collecting %d / %d episodes' % (i+1, num_episodes))
            ...
            not_dones[-1] = 0
            if i == 0:
                all_actions = actions[0:-1].copy()
                all_states = states[0:-1].copy()
                all_not_dones = not_dones[0:-1].copy()
                all_next_states = states[1:,:].copy()
            else:
                all_actions = np.concatenate((all_actions, actions[0:-1]), axis=0) 
                all_states = np.concatenate((all_states, states[0:-1]), axis=0)
                all_next_states = np.concatenate((all_next_states, states[1:]), axis=0)
                all_not_dones = np.concatenate((all_not_dones, not_dones[0:-1]))
        print('Collected actions shape:', all_actions.shape)
        print('Collected states shape:', all_states.shape)
        p.setup_dynamics(lr=params['dynamics_lr'])
        p.dynamics_steps(torch.from_numpy(all_actions), torch.from_numpy(all_states), torch.from_numpy(all_next_states), torch.from_numpy(all_not_dones), num_epochs)
        saved_model = {
                'state_dict': p.dynamics_network.state_dict(),
                'metadata': params,
                }
        torch.save(saved_model, params['dynamics_model_path'])
        ...
        for i in range(num_episodes):
            ...
            if i == 0:
                ...
                all_actions = np.concatenate((all_actions, actions), axis=0) 
                all_states = np.concatenate((all_states, states), axis=0)
            print('Episode %d / %d, Reward: %f' % (i+1, num_episodes,ep_reward))
...
def get_parser():
    ...
    # Compound training related options
    parser.add_argument('--dynamics-enable', action='store_true', help='train a dynamic model.')
    parser.add_argument('--dynamics-epochs', type=int, help='dynamics training steps.', default=200)
    parser.add_argument('--dynamics-model-path', type=str, help='path to save the dynamics network.', default='dynamics.model')
    parser.add_argument('--dynamics-lr', type=float, help='lr for dynamics learning training', default=2e-3)
    # Other configs
